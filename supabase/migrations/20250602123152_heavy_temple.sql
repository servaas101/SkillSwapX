/*
  # Fix demo user creation with proper foreign key handling
  
  1. Changes
    - Delete profile before deleting user to avoid FK constraint violation
    - Create user through auth.admin_create_user
    - Create profile and blog posts
    - Set up RLS policies
*/

-- First remove any existing profile and user to avoid conflicts
DELETE FROM public.profiles WHERE em = 'demo.author@skillswapx.com';
DELETE FROM auth.users WHERE email = 'demo.author@skillswapx.com';

-- Create demo user properly through auth system
SELECT auth.admin_create_user(
  '21e1ffb5-d6dc-47a5-aea9-0f2cd48ef758'::uuid,
