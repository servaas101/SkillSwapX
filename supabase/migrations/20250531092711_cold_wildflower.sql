...
CREATE OR REPLACE FUNCTION public.analyze_skill_gaps(p_department TEXT)
RETURNS TABLE (
  name TEXT,
  required INTEGER,
  current INTEGER,
...
BEGIN
  ...
  WITH dept_skills AS (
    ...
    WHERE p.loc = p_department -- Using location/department instead of org_id
    GROUP BY si.skill_name
  )
  SELECT 
    ds.skill_name as name,
    5 as required,
    ds.avg_level as current,
    5 - ds.avg_level as gap,
    CASE 
      WHEN ds.skill_name ILIKE '%cloud%' THEN 'Advanced Cloud Architecture'
      WHEN ds.skill_name ILIKE '%api%' THEN 'API Design Patterns'
      ELSE 'System Design Fundamentals'
    END as course,
    CASE 
      WHEN ds.skill_name ILIKE '%cloud%' THEN '8 weeks'
      WHEN ds.skill_name ILIKE '%api%' THEN '6 weeks'
      ELSE '4 weeks'
    END as duration
  FROM dept_skills ds;
END;
...
BEGIN
  ...
  WITH project_skills AS (
    SELECT DISTINCT unnest(skl) as skill_name -- Using skl from projects table
    FROM projects
  ...
  candidate_scores AS (
    SELECT 
      p.id,
      p.fn || ' ' || COALESCE(p.ln, '') as full_name,
...
DROP POLICY IF EXISTS "Users can manage own skills" ON public.skill_inventory;
DROP POLICY IF EXISTS "Users can view department skills" ON public.skill_inventory;
